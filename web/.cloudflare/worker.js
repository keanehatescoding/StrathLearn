...
var is_array, index_of, array_from, define_property, get_descriptor, noop, HYDRATION_START, HYDRATION_END, HYDRATION_ERROR, ELEMENT_IS_NAMESPACED, ELEMENT_PRESERVE_ATTRIBUTE_CASE, VOID_ELEMENT_NAMES, DOM_BOOLEAN_ATTRIBUTES, PASSIVE_EVENTS, RAW_TEXT_ELEMENTS, ATTR_REGEX, CONTENT_REGEX, replacements, current_component, BLOCK_OPEN, BLOCK_CLOSE, EMPTY_COMMENT, INVALID_ATTR_NAME_CHAR_REGEX, on_destroy;
...
function get2(key2, parse = JSON.parse) {
  try {
...
function Icon($$payload, $$props) {
  ...
  const mergeClasses = (...classes) => classes.filter((className, index4, array2) => {
    return Boolean(className) && array2.indexOf(className) === index4;
  }).join(" ");
